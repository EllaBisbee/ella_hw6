...
{
 //cout << "entering followleg\n";
  if(!(msg->people.empty()))
 {
  cout << "entering followleg data get\n";
//setup the action and goal to move the robot                                 
  ...
  double distance =  sqrt(pow(person_x - curr_x, 2) + 
			pow(person_y - curr_y, 2));                                                        
  //set goal values               
  cout << "setting goal values\n";                                              
  goal.target_pose.header.stamp = ros::Time::now();                             
  //TODO: figure out frame_id                                                   
  goal.target_pose.header.frame_id = "/base_link"; 
  if (distance >= 2)
  {                             
  	goal.target_pose.pose.position.x = curr_x + 
				(distance - 2) / distance * (person_x - curr_x);                         
  	goal.target_pose.pose.position.y = curr_y +
				(distance - 2) / distance * (person_y - curr_y);
  }
  else
  {
  	goal.target_pose.pose.position.x = (curr_x + person_x)/2;
 	goal.target_pose.pose.position.y = (curr_y + person_y)/2;
  }
  goal.target_pose.pose.position.z = person_z;
  goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(0);
  cout << "curr xy " << curr_x << " " << curr_y << " person xy " 
	<< person_x << " " << person_y << " goal xy " 
